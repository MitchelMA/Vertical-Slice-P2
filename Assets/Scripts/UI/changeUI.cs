...
public class changeUI : MonoBehaviour
{
    ...
    public void changePlayer1(){
        if(status1 == false){
            status2 = false;
            LeanTween.move(player2Long, new Vector3(630,640,0f), speed);
            LeanTween.move(player1Long, new Vector3(630,450,0f), speed).setDelay(0.5f);
            LeanTween.move(player1, new Vector3(530,-645,0f), speed);
    ...
    public void changePlayer2(){
        if(status2 == false){
            status1 = false;
            LeanTween.move(player1Long, new Vector3(630,640,0f), speed);
            LeanTween.move(player2Long, new Vector3(630,450,0f), speed).setDelay(0.5f);
            LeanTween.move(player2, new Vector3(530,-655,0f), speed);
